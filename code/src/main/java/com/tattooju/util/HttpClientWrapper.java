...
//          SSLConnectionSocketFactory  sslsf=new SSLConnectionSocketFactory(sslContext);
     ...
     * @throws IOException
     */
    public HttpResponseContent getResponse(String url) throws HttpException, IOException {
     ...
     * @throws IOException
     */
    public HttpResponseContent getResponse(String url, String urlEncoding) throws HttpException, IOException {
     ...
     * @throws IOException
     */
    public HttpResponseContent postNV(String url) throws HttpException, IOException {
        return this.getResponse(url, "UTF-8", VERBTYPE.POST, null);
    }
 
    public HttpResponseContent postNV(String url, String contentType) throws HttpException, IOException {
    ...
    public HttpResponseContent getResponse(String urlstr, String urlEncoding, VERBTYPE bodyType, String contentType)
        ...
        try {
            ...
            HttpResponseContent ret = new HttpResponseContent();
    ...
    private void getResponseContent(HttpEntity entity, HttpResponseContent ret) throws IOException {
    ...
    public HttpResponseContent postEntity(String url) throws HttpException, IOException {
     ...
     * @throws IOException
     */
    public HttpResponseContent postEntity(String url, String urlEncoding) throws HttpException, IOException {
        if (url == null)
            return null;
    ...
    * @throws IOException
    */
    public HttpResponseContent postStringEntity(String url, String urlEncoding,String contentType,String stringContent) throws HttpException, IOException {
        if (url == null)
            return null;
        ...
        try {
            ...
            StringEntity requestEntity = new StringEntity(stringContent,urlEncoding);
            ...
            HttpResponseContent ret = new HttpResponseContent();
            ret.setStatusCode(statusLine.getStatusCode());
            getResponseContent(responseEntity, ret);
            return ret;
        } finally {
            close(responseEntity, request, response);
        }
    }
     ...
     * @throws IOException
     */
     public HttpResponseContent postStringEntity(String url, String urlEncoding,Map<String,String> headerMap,String contentType,String stringContent) throws HttpException, IOException {
         if (url == null)
             return null;
    ...
    private void parseUrl(String url) {
        ...
        int pos = u.indexOf(ProtocolSeparator);//协议符号的位置://
        if (pos > 0) {
            MeetProtocol = true;
            this.protocol = u.substring(0, pos); //获取协议
            pos += ProtocolSeparator.length();
        }
        int posStartDir = 0;
        if (MeetProtocol) {
            int pos2 = u.indexOf(PortSeparator, pos);//判断是否有端口号
            if (pos2 > 0) {
                this.host = u.substring(pos, pos2);//获取主机
                pos2 = pos2 + PortSeparator.length();
                int pos3 = u.indexOf(HostSeparator, pos2);//端口号之后第一个/ http://www.ediankai.com.cn:81/openapi.php?c=w
                String PortStr = null;
                if (pos3 > 0) {
                    PortStr = u.substring(pos2, pos3);//获取端口号
                    posStartDir = pos3;
                } else {
                	//无端口号
                    int pos4 = u.indexOf("?"); //URL中的参数开始符号
                    if (pos4 > 0) {
                        PortStr = u.substring(pos2, pos4); //获取端口号
                        posStartDir = -1;
                    } else {
